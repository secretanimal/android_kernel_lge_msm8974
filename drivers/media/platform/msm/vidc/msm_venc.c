...
static struct msm_vidc_ctrl msm_venc_ctrls[] = {
	...
	},
	{
		...
		.maximum = EXTRADATA_METADATA_MBI,
		.default_value = EXTRADATA_NONE,
		.menu_skip_mask = ~(
			(1 << EXTRADATA_NONE) |
			(1 << EXTRADATA_MB_QUANTIZATION) |
			(1 << EXTRADATA_INTERLACE_VIDEO) |
			(1 << EXTRADATA_VC1_FRAMEDISP) |
			(1 << EXTRADATA_VC1_SEQDISP) |
			(1 << EXTRADATA_TIMESTAMP) |
			(1 << EXTRADATA_S3D_FRAME_PACKING) |
			(1 << EXTRADATA_FRAME_RATE) |
			(1 << EXTRADATA_PANSCAN_WINDOW) |
			(1 << EXTRADATA_RECOVERY_POINT_SEI) |
			(1 << EXTRADATA_CLOSED_CAPTION_UD) |
			(1 << EXTRADATA_AFD_UD) |
			(1 << EXTRADATA_MULTISLICE_INFO) |
			(1 << EXTRADATA_NUM_CONCEALED_MB) |
			(1 << EXTRADATA_METADATA_FILLER) |
			(1 << EXTRADATA_INPUT_CROP) |
			(1 << EXTRADATA_DIGITAL_ZOOM) |
			(1 << EXTRADATA_ASPECT_RATIO) |
			(1 << EXTRADATA_LTR) |
			(1 << EXTRADATA_METADATA_MBI)
			),
				...
				void *alloc_ctxs[])
{
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE:
		*num_planes = 1;
		...
		if ((extradata == EXTRADATA_MULTISLICE_INFO) ||
			(extradata == EXTRADATA_LTR))
